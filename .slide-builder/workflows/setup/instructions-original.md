...
Continuing with other assets...</output>
  ...
  <step n="1.8" goal="Analyze PDFs with chunked processing for large documents">
    <check if="pdf_paths is PROVIDED">
      <action>Initialize {{pdf_chunk_results}} = [] to store all chunk results for extraction</action>
      <action>Initialize {{pdf_analysis}} = "" for combined analysis</action>

      <for-each item="pdf_path" in="{{pdf_paths}}">
        <output>📄 Processing PDF: {{pdf_path}}...</output>

        <!-- Step 1.8.1: Get page count using MCP PDF reader -->
        <action>Use ToolSearch to load mcp__pdf-reader__read_pdf tool if not already loaded</action>
        <action>Call mcp__pdf-reader__read_pdf with:
          - file_path: {{pdf_path}}
          - include_page_count: true
          - include_metadata: true
          - pages: "1"  (minimal read just to get page count)
        </action>

        <check if="PDF read fails">
          <output>⚠️ Could not read PDF: {{pdf_path}} - {{error_message}}
Continuing with other assets...</output>
          <action>Mark this PDF as FAILED</action>
          <action>Continue to next PDF</action>
        </check>

        <action>Extract {{total_pages}} from PDF metadata/page count</action>
        <action>Set {{chunk_size}} = 1 (from workflow config: pdf_chunk_size - always 1 page at a time)</action>

        <!-- Step 1.8.1b: Context management strategy for images -->
        <critical>API limit: Images in multi-image requests must be under 2000px per dimension.
          Solution: Process each chunk WITH images, but extract findings immediately and don't
          accumulate raw image data in context. Process → Extract → Summarize → Move on.
          This allows full image analysis of every page without hitting context limits.</critical>

        <output>📊 PDF has {{total_pages}} pages (processing 1 page at a time)
🖼️ Strategy: Process each page with images, extract findings immediately, checkpoint every 10 pages</output>

        <!-- Step 1.8.2: Handle single-page PDF (no batching needed) -->
        <check if="{{total_pages}} == 1">
          <output>📄 Processing page 1 of 1...</output>
          ...
          <check if="API error due to image size">
            <output>⚠️ Image too large, retrying without images...</output>
            <action>Retry mcp__pdf-reader__read_pdf with include_images: false, pages: "1"</action>
          </check>

          <action>Extract brand findings from page 1</action>
          <action>Append analysis to {{pdf_analysis}}</action>

          <output>✅ PDF analysis complete: {{pdf_path}} (1 page)</output>
        </check>

        <!-- Step 1.8.3: Process multi-page PDFs - one page at a time with batched checkpoints -->
        <check if="{{total_pages}} > 1">
          <action>Initialize {{current_page}} = 1</action>
          <action>Initialize {{batch_number}} = 1</action>
          <action>Set {{pages_per_batch}} = 10 (checkpoint findings every 10 pages to avoid context overflow)</action>
          <action>Initialize {{batch_findings}} = {
            colors: [],
            typography: [],
            spacing: [],
            logo: [],
            ...
            imagery: [],
            dos_and_donts: []
          } for current batch</action>

          <action>Create checkpoint directory: {config_path}/brand-extraction-temp/</action>
          ...
          <!-- Page processing loop - one page at a time -->
          <loop while="{{current_page}} <= {{total_pages}}">

            <output>📄 Processing page {{current_page}} of {{total_pages}}...</output>

            <!-- Step 1: Probe page to check image dimensions before loading -->
            <action>Call mcp__pdf-reader__read_pdf with:
              - file_path: {{pdf_path}}
            ...
            <!-- Check if page has images that might exceed 2000px limit -->
            <action>Inspect metadata for image dimensions on this page:
              - Look for image width/height in page metadata
              - Set {{images_safe}} = true if all images <= 2000px in both dimensions
              - Set {{images_safe}} = false if any image > 2000px in either dimension
              - Set {{images_safe}} = true if no image dimension info available (try anyway)
            </action>

            <check if="{{images_safe}} is true">
              <output>  🖼️ Images safe, reading page {{current_page}} with images...</output>
              <action>Call mcp__pdf-reader__read_pdf with:
                - file_path: {{pdf_path}}
                - include_full_text: true
                - include_images: true
            ...
            <check if="{{images_safe}} is false">
              <output>  ⚠️ Large images, reading page {{current_page}} text only...</output>
              <action>Call mcp__pdf-reader__read_pdf with:
                - file_path: {{pdf_path}}
                - include_full_text: true
                - include_images: false
            ...
            <!-- Fallback if image load still fails despite check -->
            <check if="API error due to image size">
              <output>  ⚠️ Image error on page {{current_page}}, retrying text-only...</output>
              <action>Retry mcp__pdf-reader__read_pdf with:
                - file_path: {{pdf_path}}
                - include_full_text: true
                - include_images: false
                - pages: "{{current_page}}"
            ...
            <check if="page read succeeds">
              <!-- CRITICAL: Extract findings IMMEDIATELY, don't store raw content -->
              <critical>Context Management: Extract brand findings NOW, then discard raw page data.
                Only store the extracted findings, NOT the images or full text.
                This prevents context accumulation across pages.</critical>

              <action>IMMEDIATELY extract brand findings from page {{current_page}} into structured format:
                {{page_findings}} = {
                  page: {{current_page}},
                  colors: [any hex codes, color names, color usage rules found],
                  typography: [font names, sizes, weight rules, hierarchy rules],
                  spacing: [margin rules, padding guidelines, grid specs],
                  logo: [logo usage rules, clear space, placement restrictions],
                  tone_of_voice: [personality traits, writing guidelines, examples],
                  imagery: [photo style, illustration rules, icon guidelines],
                  dos_and_donts: [explicit do/don't rules found]
                }
              </action>

              <!-- Merge findings into batch structure (not raw content) -->
              <action>Merge {{page_findings}} into {{batch_findings}}:
                - Append new items to each category array
                - Don't duplicate identical findings
                - Note page number for conflict resolution later
              </action>

              <action>Update status.yaml pdf_chunking.current_page = {{current_page}}</action>

              <output>✅ Page {{current_page}}/{{total_pages}} complete - findings extracted</output>
            </check>

            <check if="page read fails">
              <output>⚠️ Failed to read page {{current_page}}: {{error_message}}
Continuing with next page...</output>
            </check>

            <!-- CHECKPOINT: Every N pages, save findings to file and reset batch -->
            <check if="{{current_page}} is divisible by {{pages_per_batch}} OR {{current_page}} == {{total_pages}}">
              <output>💾 Checkpoint: Saving batch {{batch_number}} (pages {{(batch_number-1) * pages_per_batch + 1}}-{{current_page}}) to file...</output>

              <action>Write {{batch_findings}} to {config_path}/brand-extraction-temp/batch-{{batch_number}}.json</action>

              <action>Reset {{batch_findings}} = {
                colors: [],
                typography: [],
                spacing: [],
                logo: [],
              ...
              <action>Increment {{batch_number}}</action>

              <output>✅ Batch saved. Context cleared. Continuing...</output>
            </check>

            <action>Increment: {{current_page}} = {{current_page}} + 1</action>
          </loop>

          <!-- Final synthesis: Merge all batch checkpoint files -->
          <output>📦 Merging {{batch_number - 1}} batch files into final brand knowledge...</output>

          <action>Read all batch files from {config_path}/brand-extraction-temp/batch-*.json</action>

          <action>Merge all batch findings into {{final_findings}}:
            - Combine arrays from each category across all batches
            - Deduplicate identical rules
            - Resolve conflicts (later batches/pages override earlier if more specific)
          </action>

          <action>Synthesize {{final_findings}} into coherent {{pdf_analysis}}:
            - Structure into final brand knowledge categories
            - Generate summary of key brand rules
            - Note which pages had image analysis vs text-only
          </action>

          <action>Write final merged findings to {config_path}/brand-knowledge.json</action>

          <action>Clean up: Delete {config_path}/brand-extraction-temp/ directory</action>

          <action>Update status.yaml pdf_chunking.status = "complete"</action>
          <output>✅ PDF analysis complete: {{pdf_path}}
📊 {{total_pages}} pages processed (1 page at a time, {{batch_number - 1}} checkpoint batches)
💾 Brand knowledge saved to .slide-builder/config/brand-knowledge.json</output>
        </check>
      </for-each>

      <action>Store {{pdf_chunk_results}} in session for brand knowledge extraction (Story 1.2)</action>
      <action>Update status.yaml with:
        extraction:
          pdf_analysis: complete
          pdf_chunks_processed: [count of {{pdf_chunk_results}}]
          chunks_with_images: [count where images_included == true]
      </action>
    </check>
  </step>
...
Notes: {{brand_personality.notes}}</output>
  ...
  <step n="2.6" goal="Derive typography scale">
    ...
    <critical>
      Typography scale MUST meet design standards minimums.
      Reference: .slide-builder/config/design-standards.md
    </critical>

    <action>Calculate typography scale based on design standards minimums:

      **Design Standards Minimums (for 1920x1080 at 50% scale readability):**
      ...
      {{typography_scale}} = {
        hero: "72px",    # Large impact titles (min 64px)
        h1: "48px",      # Primary headings (min 48px)
        h2: "36px",      # Secondary headings (min 36px)
        h3: "28px",      # Tertiary headings (min 28px)
        body: "24px",    # Body text (min 24px - CRITICAL for readability)
        small: "18px"    # Labels, metadata (min 18px)
      }
      ...
      Scale ratio should be 1.25-1.33 between heading levels.
    </action>

    <output>✅ Typography scale derived (meets design standards minimums)</output>
  </step>
...
Attempting to regenerate...</output>
    ...
    <action>Generate catalog.json manifest at {config_path}/catalog/slide-templates.json with:
      ```json
      {
        "version": "1.0",
    ...
    <action>Update status.yaml with:
      ...
      catalog:
        ...
        manifest: .slide-builder/config/catalog/slide-templates.json
        count: 6
        templates:
          - id: title
            name: Title Slide
...
- .slide-builder/config/catalog/slide-templates/title.html
- .slide-builder/config/catalog/slide-templates/agenda.html
- .slide-builder/config/catalog/slide-templates/process-flow.html
- .slide-builder/config/catalog/slide-templates/comparison.html
- .slide-builder/config/catalog/slide-templates/callout.html
- .slide-builder/config/catalog/slide-templates/technical.html
- .slide-builder/config/catalog/slide-templates.json (lastModified updated)
...
1. **Single slide:** Run `/sb-create:plan-one` to create one slide quickly
2. **Full deck:** Run `/sb-create:plan-deck` to plan a complete presentation
3. **View theme:** Run `/sb-brand:theme` to see your theme summary anytime
4. **Edit theme:** Run `/sb-brand:theme-edit` to modify your theme later
5. **Add template:** Run `/sb-manage:add-slide-template` to create custom templates
